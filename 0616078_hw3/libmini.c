...
void _memset(void *s, size_t size, int val)
{
    char *ptr = (char *)s;
    while(size-- > 0) *ptr++ = val;
}

void _memcpy(void *s, const void *t, size_t size)
{
    char *ptr1 = (char *)s;
    char *ptr2 = (char *)t;
...
int sigprocmask(int how, const sigset_t *set, sigset_t *oldset)
{
    ...
    {
        sigset_t tmpset;
        _memcpy(&tmpset, set, sizeof(sigset_t));
        ret = sys_rt_sigprocmask(how, &tmpset, oldset, sizeof(sigset_t));
    }
    else
...
int sigaction(int signum, const struct sigaction *act, struct sigaction *oldact)
{
    ...
    if (act)
        ...
        _memcpy(&kact.sa_mask, &act->sa_mask, sizeof (sigset_t));
    ...
    if (oldact && ret >= 0)
        ...
        _memcpy(&oldact->sa_mask, &koact.sa_mask, sizeof(sigset_t));
...
sighandler_t signal(int signum, sighandler_t handler)
{
    ...
    if(handler == SIG_ERR || signum <= 0 || signum >= NSIG)
    {
        SET_ERRNO(EINVAL);
        return SIG_ERR;
    }
